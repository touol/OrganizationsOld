 ...
 * Create an Invite
 */
class OrganizationsInviteCreateProcessor extends modObjectCreateProcessor {
	...
	public function process() {
		...
		switch($data['type']){
			case 1:
				$data['invite_code'] = $Orgs->generate(6, 'IO1-');
			break;
			case 2:
				$data['invite_code'] = $Orgs->generate(6, 'IO2-');
				if($data['period']!=''){
					$data['date_exp']= strftime('%Y-%m-%d %H:%M:%S',strtotime('now +'.$data['period'].' month'));
				}
			break;
			default:
				$data['invite_code'] = $Orgs->generate(6, 'DEF-');
			break;
		}
...
return 'OrganizationsInviteCreateProcessor';